...
int VoxelWorld::register_perset(const Ref<Perset> value)
{
	...
	for (size_t i = 0; i < material_persets.size(); i++)
	{
		if (material_persets[i].get_type() == Variant::NIL) {
			perset_map[uuid] = i;
			material_persets[i] = value;
			return i;
		}
	}
	material_persets.push_back(value);
	perset_map[uuid] = material_persets.size() - 1;
	return material_persets.size() - 1;
}
...
Array VoxelWorld::get_mesh_persets() const
{
	return mesh_persets;
}
...
uint8_t VoxelWorld::get_voxel_type(const Voxel& voxel)
{
	return (voxel >> 6) & 0b00000011;
}

uint8_t VoxelWorld::get_voxel_id(const Voxel& voxel)
{
	return voxel >> 8;
}

uint8_t VoxelWorld::get_voxel_flag(const Voxel& voxel)
{
	return voxel & 0b00111111;
}

Vector3i VoxelWorld::flag_to_rotation(const uint8_t& flag)
{
